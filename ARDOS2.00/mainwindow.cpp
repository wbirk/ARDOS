...
MainWindow::MainWindow(QWidget *parent):QMainWindow(parent),ui(new Ui::MainWindow) {
    ...
    this->endSwitchHit[0] = this->endSwitchHit[1] = this->endSwitchHit[2] = 0; // it is assumed that endswitches are not hit
    ...
    for (driveCounter = 0; driveCounter < 4; driveCounter++) {
        ...
        serNo[driveCounter] = StepperDrive[driveCounter]->retrievePhidgetStepperData(1); // get the serial number of the stepper controllers
...
//--------------------------------------------------------------------------------------
// position the drives 1-3 to endswitches

void MainWindow::goToStartPosition(void) {
    double speeds[3];
    short driveCount, digitalInputDr1,digitalInputDr2,digitalInputDr3;
    QElapsedTimer *wait;
    ...
    speeds[1] = 5000;
    speeds[2] = 5000;
    for (driveCount = 0; driveCount < 3; driveCount++) {
    ...
    } while (this->endSwitchHit[2] == true);  // repeating the procedure for all other drives
    ...
    this->StepperDrive[0]->sendSteps(110000); // drive 1 has a 1:4.25 gearbox
    QCoreApplication::processEvents(QEventLoop::AllEvents, 100);
    this->StepperDrive[1]->sendSteps(15060);
    QCoreApplication::processEvents(QEventLoop::AllEvents, 100);
    this->StepperDrive[2]->sendSteps(19700); // move drives further away from endswitches
    QCoreApplication::processEvents(QEventLoop::AllEvents, 100);
...
// now start the periodic motion for all four drives
            ...
            if (this->loopWasTriggered == true) {
                if (ADInterface->getDigitalInput(6) == 1) {
                    emergencyStop();
                    this->loopWasTriggered = false;
                }
            }
...
// next line: step2 and so on for drive 1 ...
...
void MainWindow::saveTrajectoryFile(void) {
    ...
    this->trajectoryFile = new QFile();
    this->trajectoryFile->setFileName("ARDOSTrajectoryFile.csv");
